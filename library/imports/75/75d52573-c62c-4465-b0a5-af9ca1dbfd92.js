"use strict";
cc._RF.push(module, '75d52VzxixEZbClr5yh2/2S', 'game');
// game.js
...
cc.Class({
	...
	shake: function shake() {
		// 爪子开始左右晃动
		this.shake_zhuazi_seq = cc.repeatForever(cc.sequence(cc.moveBy(1, -100, 0), cc.moveBy(1, 100, 0), cc.moveBy(1, 100, 0), cc.moveBy(1, -100, 0)));
		this.shake_zhualine_seq = cc.repeatForever(cc.sequence(cc.moveBy(1, -100, 0), cc.moveBy(1, 100, 0), cc.moveBy(1, 100, 0), cc.moveBy(1, -100, 0)));
		this.shake_zhuatop_seq = cc.repeatForever(cc.sequence(cc.moveBy(1, -100, 0), cc.moveBy(1, 100, 0), cc.moveBy(1, 100, 0), cc.moveBy(1, -100, 0)));
		...
		this.shake_wawa_seq = cc.repeatForever(cc.sequence(cc.moveBy(2, -(container_width / 3), 0), cc.moveBy(2, container_width / 3, 0), cc.moveBy(2, container_width / 3, 0), cc.moveBy(2, -(container_width / 3), 0)));
	...
	sendHttpReq: function sendHttpReq(url, data, _cb) {
		var that = this;
	...
	getLotteryId: function getLotteryId() {
	...
	initData: function initData() {
		...
		this.sendHttpReq(url, data, function (respObj, resp, gameObj) {
			...
			for (var i = 0; i < parseInt(respObj['data']['good_num']); i++) {
				...
				wawa.getComponent('good').setData(respObj['data']['lottery_good' + i]['name'], respObj['data']['lottery_good' + i]['img_url']);
	...
	zhuawawa_go: function zhuawawa_go() {
		...
		var zhuazi_action = cc.sequence(cc.moveTo(0.1, target_x, target_y), cc.rotateTo(0.1, rotateX), cc.rotateTo(0.1, 0.0), cc.rotateTo(0.1, -rotateX), cc.rotateTo(0.1, 0.0));
		this.zhuazi.node.runAction(zhuazi_action);
		var that = this;
		...
		var target_y = this.orign_zhualine_pos.y + this.zhua_line.node.height / 2 - this.orign_zhualine_scale_y * this.zhua_line.node.height / 2;
		var zhualine_x = this.zhua_line.node.x;
		var zhualine_action = cc.spawn(cc.scaleTo(0.1, 1, this.orign_zhualine_scale_y), cc.moveTo(0.1, zhualine_x, target_y));
	...
	zhuawawa: function zhuawawa() {
		...
		var that = this;
		var rotateX = 2;
		var zhuazi_action = cc.sequence(cc.rotateTo(0.1, rotateX), cc.rotateTo(0.1, 0.0), cc.rotateTo(0.1, -rotateX), cc.rotateTo(0.1, 0.0), cc.rotateTo(0.1, rotateX), cc.rotateTo(0.1, 0.0), cc.rotateTo(0.1, -rotateX), cc.rotateTo(0.1, 0.0), cc.rotateTo(0.1, rotateX), cc.rotateTo(0.1, 0.0));
		this.zhuazi.node.runAction(zhuazi_action);
		setTimeout(function () {
			that.zhuawawa_go();
		}, 1200);
