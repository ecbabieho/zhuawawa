...
function init_share(){
	...
	$.post(url,data,function(ret){
		if (ret['code'] == 0) {
			...
			share_data['title'] = "哐糖会员好物分享，这里的优惠服务超赞，快来看看~";
			share_data['imgUrl'] = window.location.protocol + "//" + window.location.host + "/Public/weixin/image/v_share.png";
			share_data['link'] = ret['data']['signPackage']['url'];
			share_data['desc'] = "全城福利，1块钱一网打尽~";
			fill_share_data();
		}
		else{
			alert("获取分享信息失败");
